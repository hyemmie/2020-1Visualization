...
ggplot(iris, aes(x=Sepal.Length)) +
  geom_histogram()
#수치형데이터의 경우 stat="count" 안 들어가도 됨
...
df<- read_csv("data/line_temp.csv")
View(df)

ggplot(df %>% filter(category %in% c("max_temp","min_temp"),
                     month %in% c(7,8,9)), aes(x=dates, y=value, col=category)) +
  geom_line() +
  ylim(20,40)
#------------------------------------------------
# Scatteor Plots
...
ggplot(df_temp, aes(x = dates, y = value, color = category)) +
  ...
  labs(title="최고온도 꾸준히 상승 중",
       subtitle="최저온도는 하향세 보여",
       caption="출처:기상청",
       x=NULL,
       y=NULL) + 
