...
#df_weather$category <- str_remove_all(df_weathe$category)

#df_weather %>% 
#  select(dates, starts_with("2018")) %>% 
#  str_rm
#  spread(key=category,value=)

ggplot(df_weather, aes(x=dates, y=value, group= category, col= category)) +
  geom_point() +
  stat_smooth(values="lm") +
  theme_fivethirtyeight(base_family = "NanumGothic") +
  labs(title = "여름 최고/최저기온 변화", 
       subtitile = "1960년부터 2018년까지", 
       caption = "출처 : 정보공개청구") +
  facet_grid(. ~ category )


df_weather_min = df_weather %>% filter (value==min(value))  
